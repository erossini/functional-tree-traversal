...
namespace DataLayer.BinaryTree
{
    ...
    public class Node
    {
        ...
        public Node(NodeData value, Node left, Node right)
        {
            Right = left;
            Left = right;
            Value = value;
        ...
        public bool Add(Node node)
        {
            ...
            if (Value != null)
            {
                if (node.Value.Age < Value.Age) // add to left : it's value is less than the value of the current node
                {
                    res = true;
                    if (Left == null)
                        Left = node;
                ...
                if (node.Value.Age > Value.Age) // add to right: it's value is greater than the value of the current node
                {
                    res = true;
                    IsChanged = true;
                    if (Right == null)
        ...
        public bool Remove(Node nodeValue, out bool containedOnMe)
        {
            Node nodeToRemove;
            containedOnMe = false;
            // search for the node containing the nodeValue
            var isLeft = nodeValue.Value.Age < Value.Age;
            if (nodeValue.Value.Age < Value.Age)
                nodeToRemove = Left;
            else if (nodeValue.Value.Age > Value.Age)
                nodeToRemove = Right;
            else
            {
                if (Left != null)
            ...
            if (containOnChild) // if the child of the current node contained the nodeValue, that child should be removed.
            {
                ...
                else                                                        // left and right are not null
                {
                    ...
                    else                                                    // 4. the removing nodes' right child has left child
                    {
                        ...
                        Remove(theMostLeftChild, out temp);
        ...
        private static Font font = new Font("Tahoma", 8f * Coef);
